...
void test4(void) {
    ...
    //Pop the task from the linked list
    dd_task_node_t *task = pop(&list);

    //Check if the task is in the linked list
    assert(task != NULL, "Task not in the linked list");
    assert(task->task.task_id == 0, "Task id is not correct");
    assert(task->task.size == 10, "Task size is not 10");
    print_list(&list);
}
