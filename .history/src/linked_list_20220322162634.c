...
void test2(void) {
    ...
    assert(list.head->task_id == 1, "Task id is not correct");
    print_list(&list);
    free_list(&list);
}
...
void test4(void) {
    ...
    dd_task_node_t *curr = list.head;
    while (curr != NULL) {
        assert(curr->task_id != 5, "Task id is not 5");
        curr = curr->next;
    }

    //Get the id of the first element in the linked list
    int head_id = list.head->task_id;
    ...
    curr = list.head;
    while (curr != NULL) {
        assert(curr->task_id != head_id, "Task id is wrong");
        curr = curr->next;
    }
    ...
    curr = list.head;
    while (curr != NULL) {
        assert(curr->task_id != tail_id, "Task id is wrong");
        curr = curr->next;
    }
    free_list(&list);
}
