 ...
 *  so that the earliest absolute deadline is first
 * @param: list - pointer to the linked list
 * @return: void
 */
void sort_by_deadline(dd_task_list_t *list) {
    dd_task_node_t *curr = list->head;
    dd_task_node_t *prev = NULL;
    //Iterate through the list and find the earliest deadline
    //and swap it with item in the ith position
    for(int i = 0; i < list->size; i++) {
        dd_task_node_t *min = curr;
        dd_task_node_t *min_prev = prev;
        for(int j = i; j < list->size; j++) {
            if(curr->task.deadline < min->task.deadline) {
                min = curr;
                min_prev = prev;
            }
            prev = curr;
        ...
        //Swap the earliest deadline with the ith position
        if(min != curr) {
            if(min_prev == NULL) {
                list->head = min;
            } else {
