...
#define USER_DEFINED_TASK_PRIORITY	( tskIDLE_PRIORITY + 2 )
#define MONITOR_TASK_PRIORITY		( tskIDLE_PRIORITY )
#define DDS_TASK_PRIORITY			( tskIDLE_PRIORITY + 3 )
#define TIMER_PRIORITIES			( tskIDLE_PRIORITY + 1 )
...
int main(void){
	...
	xQueue_completed_dd_task = xQueueCreate(mainQUEUE_LENGTH, sizeof(uint32_t)); //TODO change to uint32_t
	...
	//dd_task_t new_task = xTaskCreate(User_Defined_Tasks_Task, "User_Defined_Tasks_Task", configMINIMAL_STACK_SIZE, &new_task, 1, &(new_task.t_handle));
...
void update_priorities(dd_task_list_t *active_task_list)
{
	...
	vTaskPrioritySet(head->task.t_handle, 2);
...
void upgrade_monitor_task_priority(TaskHandle_t monitor_t_handle){
	vTaskPrioritySet(monitor_t_handle, 4);
}
	...
	for(;;){
		if(xQueueReceive(xQueue_new_dd_task, &new_task, 0)){ //New task received
			...
			if(new_task.user_task_id == 1) {
				...
				configMINIMAL_STACK_SIZE, &new_task, tskIDLE_PRIORITY, &(task_list_task->t_handle));
		...
		if(active_task_list.size > 0){
			...
			if(xTaskGetTickCount() > head->task.absolute_deadline){
				...
				//upgrade_monitor_task_priority(monitor_t_handle);
				print_list(&active_task_list, "Overdue; Active");
			}
		}
...
dd_task_list_t get_active_dd_task_list(void)
{
	dd_task_list_t active_task_list;
	init_task_list(&active_task_list);
	if(xQueueSend(xQueue_request_active_task_list, &active_task_list, 1000) == pdTRUE);
	if(xQueueReceive(xQueue_active_task_list, &active_task_list, 1000) == pdTRUE);
